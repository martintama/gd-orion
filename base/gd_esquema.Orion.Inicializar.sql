...
alter table orion.consumos add constraint fk_consumos_idcompra foreign key (idcompra_estado) references orion.compras (idcompra_estado)
...
alter table orion.gift_cards add constraint fk_gift_cards_idcliente_origen foreign key (idcliente_destino) references orion.clientes (idcliente)
...
-- Carga completa clientes, giftcards y cargas, tipos_pago, ciudades, clientes_ciudades:	03:12
...
--Ciudades			00:00
insert into ORION.ciudades(descripcion) select distinct localidad from ORION.clientes

-- Borro esta columna ya que no la necesito más 

alter table orion.clientes drop localidad
